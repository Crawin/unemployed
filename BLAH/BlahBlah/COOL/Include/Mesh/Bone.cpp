...
void Bone::LoadBone(ComPtr<ID3D12GraphicsCommandList> commandList, std::ifstream& file, ResourceManager* manager)
{
	...
	m_BoneDataIdx = manager->CreateBufferFromVector(commandList, m_Bones, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, std::format("bone_{}", m_Name), RESOURCE_TYPES::VERTEX);
}
